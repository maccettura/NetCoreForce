...
public class ForceClient
{
    ...
    /// <summary>
    /// Create multiple ISfModel records atomically
    /// </summary>
    /// <param name="sfModels">ISfModel objects to create. Each object should have the entity type in the attributes property object.</param>
    /// <param name="customHeaders">Custom headers to include in request (Optional). The HeaderFormatter helper class can be used to generate the custom header as needed.</param>
    /// <param name="autoFillAttributes">Automatically populate attribute object property with the object's SObjectTypeName, reference Id will be the zero-based index of the array</param>
    /// <param name="fieldsToNull">A list of properties that should be set to null, but including the null values in the serialized output</param>
    /// <param name="ignoreNulls">Use with caution. By default null values are not serialized, this will serialize all explicitly nulled or missing properties as null</param>
    /// <returns>List of CompositeCreateResponse objects, includes new object IDs, and errors if any</returns>
    /// <exception cref="ForceApiException">Thrown when creation fails</exception>
    public async Task<List<CompositeCreateResponse>> CreateMultiple(
        List<object> sfModels,
        bool autoFillAttributes = true,
        Dictionary<string, string> customHeaders = null,
        List<string> fieldsToNull = null,
        bool ignoreNulls = true)
    {
        if (sfModels == null)
        {
            throw new ArgumentNullException(nameof(sfModels));
        }

        if (sfModels.Count > 200)
        {
            throw new ForceApiException($"A maximum of 200 records can be created in a single request - request included {sfModels.Count} records.");
        }

        if (autoFillAttributes)
        {
            for (int i = 0; i < sfModels.Count; i++)
            {
                var record = sfModels[i];
                var recordType = record.GetType();

                // Get the SObjectTypeName from the ISfModel interface (static abstract property)
                var bindingFlags = System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Static | System.Reflection.BindingFlags.IgnoreCase;
                var sfModelProperty = recordType.GetProperty("SObjectTypeName", bindingFlags);

                if (sfModelProperty == null)
                {
                    throw new ForceApiException($"Object type '{recordType.Name}' does not implement ISfModel or does not have a SObjectTypeName property");
                }

                // Get the SObjectTypeName value
                var sObjectTypeName = sfModelProperty.GetValue(null) as string;
                if (string.IsNullOrEmpty(sObjectTypeName))
                {
                    throw new ForceApiException($"SObjectTypeName for '{recordType.Name}' is null or empty");
                }

                // Set the Attributes property
                var instanceBindingFlags = System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.IgnoreCase;
                var attributesProperty = recordType.GetProperty("Attributes", instanceBindingFlags);

                if (attributesProperty == null)
                {
                    throw new ForceApiException($"Object type '{recordType.Name}' does not have an Attributes property");
                }

                if (!attributesProperty.CanWrite)
                {
                    throw new ForceApiException($"Object type '{recordType.Name}' does not have a writable Attributes property");
                }

                attributesProperty.SetValue(record, new SObjectAttributes()
                {
                    ReferenceId = i.ToString(),
                    Type = sObjectTypeName
                });
        ...
        {
            foreach (var obj in sfModels)
            {
                var recordType = obj.GetType();
                var instanceBindingFlags = System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.IgnoreCase;
                var attributesProperty = recordType.GetProperty("Attributes", instanceBindingFlags);

                if (attributesProperty == null)
                {
                    throw new ArgumentException($"Object type '{recordType.Name}' does not have an Attributes property");
                }

                var attributes = attributesProperty.GetValue(obj) as SObjectAttributes;
                if (attributes == null || string.IsNullOrEmpty(attributes.ReferenceId) || string.IsNullOrEmpty(attributes.Type))
                {
                    throw new ArgumentException($"All objects in request must include a reference id and sObject type name in the attributes");
                }
            }
        ...
        // Use the SObjects Composite endpoint for creating heterogeneous records
        var uri = UriFormatter.SObjectsComposite(InstanceUrl, ApiVersion);

        JsonClient client = new JsonClient(AccessToken, SharedHttpClient);

        ISfModelTreeRequest treeRequest = new ISfModelTreeRequest(sfModels);

        var result = await client.HttpPostAsyncPolymorphic<List<CompositeCreateResponse>>(treeRequest, uri, headers, fieldsToNull: fieldsToNull, ignoreNulls: ignoreNulls).ConfigureAwait(false);
    ...
    public async Task<List<UpsertResponse>> UpdateRecords(
        ...
        bool ignoreNulls = true)
    {
        ArgumentNullException.ThrowIfNull(sfModels);
        ...
        UpsertRequest2 upsertRequest = new(sfModels, allOrNone);

        var result = await client.HttpPatchAsyncPolymorphic<List<UpsertResponse>>(upsertRequest, uri, headers, fieldsToNull: fieldsToNull, ignoreNulls: ignoreNulls).ConfigureAwait(false);
